...
- `config.py` — 출력 위치 설정. `config.json`(gitignore)의 `output_dir` > env `STOCK_AGENT_VAULT` > `base_dir`. **데이터(json·briefs)는 프로젝트 유지, 사람이 읽는 문서(리포트·매매일지)만** 외부(Obsidian 볼트 등)로 출력. 볼트면 `리포트/`·`매매일지/` 한글 폴더 관례. 현재 `d:/obsidian/옵시디언/투자 노트`로 설정됨.
